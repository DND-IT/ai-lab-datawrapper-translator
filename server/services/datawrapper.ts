...
export async function getMeta(chartId: string): Promise<any> {
...
export async function getChartTable(chartId: string, metadata?: any): Promise<ChartTableData> {
  if (!metadata) metadata = await getMeta(chartId);
...
export function buildCsvWithNewLabels(rawCsv: string, metadata: any, newLabels: string[]): string {
  const firstLine = rawCsv.split('\n')[0];
  const tabCount = (firstLine.match(/\t/g) || []).length;
  const commaCount = (firstLine.match(/,/g) || []).length;
  const sep = tabCount > commaCount ? '\t' : ',';
...
export async function getMapData(mapId: string): Promise<any> {
  const res = await fetch(`${DW_API_URL}${mapId}/data`, { headers: headers('application/json') });
  await checkStatus(res);
  const data = await res.json();
  if (!data.markers) throw new Error('Map data contains no marker');
  return data;
}

// --- Write data ---

export async function putChartDataEncoded(chartId: string, data: string): Promise<void> {
...
export async function putData(mapId: string, data: any): Promise<boolean> {
...
export async function duplicateChart(chartId: string): Promise<{ new_chart_id: string; chart_type: string } | false> {
  ...
  if (typeof newChartId !== 'string') return false;
...
export async function getChartTexts(chartId: string): Promise<Record<string, string>> {
  const res = await fetch(`${DW_API_URL}${chartId}`, { headers: headers('application/json') });
  await checkStatus(res);
  ...
  if (notes) result['bottom-note'] = notes;
...
export async function patchChartMeta(chartId: string, payload: any): Promise<boolean> {
...
export async function patchMapMeta(mapId: string, targetLang = 'FR'): Promise<boolean> {
