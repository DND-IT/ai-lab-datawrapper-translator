...
export class Chart {
  ...
  metadata: any = null;
  chartType: string;
  dataTable: dw.ChartTableData | null = null;
  experimentalMode: boolean;

  constructor(chartId: string, chartType = '', experimentalMode = false) {
  ...
  async fetchDataTable(): Promise<dw.ChartTableData> {
    if (!this.dataTable) {
  ...
  async createTranslationTable(
    ...
    try {
      const sourceTextDict: Record<string, string> = await dw.getChartTexts(this.chartId);
