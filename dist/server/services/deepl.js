...
export function addLineBreaks(title) {
    if (typeof title !== 'string')
        return title;
    if (title.length <= config.maxLineLength)
        return title;
...
export async function getDeeplDictionary(cleanTitles, sourceLang = 'DE', targetLang = 'FR') {
    ...
    const data = await res.json();
    const translations = data.translations.map((t) => capitalizeFirstLetter(t.text));
    const result = {};
