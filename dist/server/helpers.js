...
export async function checkStatus(response, shouldRaise = true) {
    if (!response.ok) {
...
export async function slackAlert(message, mention = false) {
    if (!config.slackHookTechnical)
        return;
    if (mention)
        message += ' <@U020FU609LG>';
...
export async function slackInfo(message) {
    if (!config.slackHookGeneral)
        return;
