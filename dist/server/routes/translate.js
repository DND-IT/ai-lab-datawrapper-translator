...
const router = Router();
function isValidId(dwId) {
    return /^[A-Za-z0-9]{5}$/.test(dwId);
}
function makeResponse(res, payload = {}, message = '', code = 200) {
    if (code === 400)
...
router.post('/', async (req, res) => {
    try {
        ...
        let pipeline = body.pipeline || 'default';
        const step = body.step != null ? parseInt(body.step) : -1;
        const baseChartId = body.base_chart_id || '';
        const newChartId = body.new_chart_id || '';
        const chartType = body.chart_type || '';
        const translations = body.translations || [];
        const sourceLang = body.source_lang || 'DE';
        const targetLang = body.target_lang || 'FR';
        if (experimentalMode)
            console.log('Experimental mode on!');
        if (sourceLang)
            console.log('Source lang is', sourceLang);
        if (targetLang)
            console.log('Target lang is', targetLang);
        if (chartType)
        ...
        if (getUsage) {
            ...
            if (!usage)
                return makeResponse(res, {}, 'could not get API usage', 500);
        ...
        if (step === 2) {
            ...
            if (!newChartId)
                return makeResponse(res, {}, 'new_chart_id is empty', 400);
        ...
        if (step === 3) {
            if (!newChartId)
                return makeResponse(res, {}, 'new_chart_id not set', 400);
            if (pipeline === 'default') {
